<script lang="ts">
  import { ActiveRequestStatus, FailedRequestStatus, type RequestStatus } from "$lib/type"

  export let state: RequestStatus | undefined;

  let statesList = Object.keys(ActiveRequestStatus)

  let indexOfCurrentState = state ? statesList.indexOf(state) : -1;
  console.log(state)
</script>

<ol class="list-none">
  {#if Object.keys(FailedRequestStatus).some(s => s === state) || indexOfCurrentState === -1}
      <span class="p-2.5 mx-5 bg-red-500 text-white rounded-md">
        {state ?? FailedRequestStatus.Unknown}
      </span>
    {:else}
